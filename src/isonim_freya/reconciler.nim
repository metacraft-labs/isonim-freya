## isonim_freya/reconciler.nim
##
## NH-M3 — the Freya instance of IsoNim's ``RendererReconciler`` contract
## (``isonim/native/reconciler``).
##
## ## Difficulty, and where it actually lies
##
## The design doc grades Freya "medium": element identity has to be
...
## edit did not touch receives no ``freya_set_attribute`` /
## ``freya_set_style`` call at all and its cached computed style stands.
## The engine, not this file, enforces that: see ``reconcileNode``'s
## ``propsDiffer`` guard.
##
## ## What ``properties`` can and cannot see, stated plainly
##
## ``FreyaElement`` is an opaque handle into the Rust shadow tree and the
## shim exports **no enumerate-attributes entry point** — only
## ``freya_get_attribute(name)``. So ``properties`` returns the values of
## a TRACKED SET of names rather than everything the node carries, and
## ``newTrackedFreyaReconciler`` is how a caller extends it.
##
...
## alternative — a new ``freya_attribute_names`` export — is a shim + Nim
...
when defined(js):
  {.error: "isonim_freya/reconciler is for native (nim c) targets only.".}
...
proc newFreyaReconciler*(trackedProps: seq[string] = defaultTrackedProps):
    RendererReconciler[FreyaElement] =
  ## The reconciler for ``FreyaRenderer``'s shadow tree.
  ##
  ...
  RendererReconciler[FreyaElement](
    nodes: RendererTreeNodeOps[FreyaElement](
      identityKey: proc(n: FreyaElement): NodeIdentity =
        freyaIdentityKey(n, indexInParent(n)),
      kind: proc(n: FreyaElement): NodeKind =
        if n == nil: "" else: getTag(n),
      children: proc(n: FreyaElement): seq[FreyaElement] =
        result = @[]
        if n == nil: return
        for i in 0 ..< childCount(n):
          result.add(nthChild(n, i)),
      properties: proc(n: FreyaElement): Table[string, string] =
        ...
        # optimisation. ``freya_get_text_content`` AGGREGATES the text of
    ...
    placeAt: proc(parent, child: FreyaElement; index: int) =
    ...
    move: proc(parent, child: FreyaElement; fromIndex, toIndex: int) =
      if parent == nil or child == nil: return
    ...
    remove: proc(parent, child: FreyaElement) =
      if parent == nil or child == nil: return
      r.removeChild(parent, child),
    updateProps: proc(node: FreyaElement;
