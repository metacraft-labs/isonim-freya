...
proc ensureDispatcherRegistered() =
  if not dispatcherRegistered:
    freya_set_event_dispatcher(globalDispatcher)
...
proc addEventListener*(r: FreyaRenderer; node: FreyaElement; event: string; handler: proc()) =
  ...
  ## Uses the callback ID registry + dispatcher to bridge Nim closures.
  let callbackId = registerCallback(handler)
  freya_add_event_listener_id(node, event.cstring, callbackId)
