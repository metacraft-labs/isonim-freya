//! Window management and event loop integration for the Freya shim.
//!
...
//!
//! When the `freya-backend` feature is enabled, `show_window` will launch an
//! actual Freya window with event loop. Without the feature, the window state
...
/// Global repaint flag. When set to true, the Freya event loop (if running)
/// should trigger a redraw on the next frame. Tree-manipulating functions
/// set this flag, and the render loop clears it after processing.
...
///
/// Without `freya-backend`, this just updates the state.
/// With `freya-backend`, this would launch the Freya event loop (blocking).
///
...
mod tests {
    ...
    fn test_close_callback_prevents_close() {
        ...
        // Register a callback that prevents closing
