## Task Manager demo app — FreyaRenderer version.
##
## This is a port of the isonim-replica demo app that uses FreyaRenderer
## instead of the DOM renderer. It demonstrates all task management
...
##   - Window mode (-d:freyaGui): creates an actual Freya window.
##
## Build & run:
...
# ============================================================================
# UI building with FreyaRenderer (imperative reactive style)
# ============================================================================
...
type
  TaskApp* = ref object
    ...
    root*: FreyaElement
    store*: TaskStore
    inputField*: FreyaElement
    addBtn*: FreyaElement
    taskListSection*: FreyaElement
    footerContainer*: FreyaElement
    filterBtnAll*: FreyaElement
    filterBtnActive*: FreyaElement
    filterBtnCompleted*: FreyaElement
    clearCompletedBtn*: FreyaElement
    countLabel*: FreyaElement
    inputValue*: Signal[string]

proc buildTaskApp*(r: FreyaRenderer): TaskApp =
  ## Builds the complete task manager UI tree using FreyaRenderer.
...
proc dumpTree*(node: FreyaElement; indent: int = 0): string =
...
when isMainModule:
  ...
  when defined(freyaGui):
    ...
    createRoot proc(dispose: proc()) =
      ...
      discard win.show()
      # Event loop would go here with Freya's launch
      echo "Window mode not yet fully implemented (requires freya-backend feature)"
      dispose()
