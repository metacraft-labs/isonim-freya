## NH-M3 — ``test_freya_reconciler_preserves_identity``
##
...
## ``freya_*`` call below crosses the FFI into ``libfreya_nim_shim``), the
...
## went green without the shim would be asserting nothing about Freya.
##
## ## Why identity is asserted with ``sameNode`` and not ``==``
##
## ``FreyaElement`` is an opaque handle, and **every child/parent accessor
...
## ``sameNode`` compares ``freya_node_id``, which is the identity.
##
## ## Discrimination
##
...
when not defined(isonimHmr):
  {.error: "test_freya_reconciler_identity must be compiled with " &
...
import isonim_freya/reconciler
...
const
  slotRows = "freya_recon_demo.nim:21:2"

var r: FreyaRenderer
var rowsVersion = 1

proc keyed(tag, key: string; text: string = ""): FreyaElement =
...
proc makeRows(version: int): UiSlotFactory =
  ...
  uiSlotFactory(proc(): FreyaElement =
...
proc demoRoot(): FreyaElement =
  let root = keyed("div", "root")
  r.appendChild(root, hmrInvokeSlot[FreyaElement](slotRows))
  root

proc detachedRootAt(version: int): FreyaElement =
  ...
  let root = keyed("div", "root")
  r.appendChild(root, unboxUiNode[FreyaElement](makeRows(version)()))
  root

proc childByKey(n: FreyaElement; key: string): FreyaElement =
  if n == nil: return nil
...
proc deepChild(n: FreyaElement; path: varargs[string]): FreyaElement =
  result = n
  for key in path:
...
proc newAlwaysReplaceReconciler(): RendererReconciler[FreyaElement] =
  result = newFreyaReconciler()
  result.nodes.identityKey = proc(n: FreyaElement): NodeIdentity =
...
proc runReload(rec: RendererReconciler[FreyaElement];
  ...
  stub.queuePatch(HcrStubPatch(
    changedFiles: @["freya_recon_demo.nim"],
    changedTypes: @[],
    applyCodeSwap: proc() =
...
suite "NH-M3: Freya reconciler preserves identity":

  test "test_freya_reconciler_preserves_identity":
    let outcome = newFreyaReconciler().runReload(bumpRowsHash = true)
  ...
  test "an IDENTICAL rebuild touches the renderer zero times":
    ...
    # SAME node the old root is holding, and `freya_append_child`
    ...
    var stats = ReconcileStats()
    let surviving = newFreyaReconciler().reconcile(before, after, stats)
  ...
  test "RECORDED: freya_append_child RE-PARENTS, which is why a memo-built root cannot be reconciled in place":
  ...
  test "the Freya instance satisfies the whole contract":
    newFreyaReconciler().validate()
