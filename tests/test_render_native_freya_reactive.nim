## NH-M1 verification: test_render_native_freya_reactive
##
## Claim under test (from
## `codetracer-specs/Front-Ends/IsoNim/Hot-Module-Reload-Native.milestones.org`,
## NH-M1): the Freya mount path runs a render effect, so mutating a signal the
## root proc reads rebuilds and re-inserts the root — "same against Freya's
## element tree".
##
## MOCK POLICY: no mocks. Every element here is created through
## `FreyaRenderer`, i.e. through the real `freya_*` FFI into the Rust shim
## (`libfreya_nim_shim`); the assertions read the tree back with
## `freya_child_count` / `freya_get_text_content` through `childCount` and
...
## `{.dynlib: libfreya_nim_shim.so.}`. `bindings.nim` bakes in the absolute
...
## body with "could not load: libfreya_nim_shim.so" — a hard non-zero exit, and
...
## about Freya. Build it with `just rust-build` first.
##
...
import isonim_freya/reactive_root

import isonim/core/signals

proc buildLabelTree(r: FreyaRenderer; text: string): FreyaElement =
...
suite "NH-M1: renderFreya reactive root (isonim-freya)":

  test "test_render_native_freya_reactive":
    freya_reset_tree()
    resetCallbacks()
    let r = FreyaRenderer()
    ...
    let handle = renderFreya(r, host,
      NativeRootAccessor[FreyaElement](proc(): FreyaElement =
    ...
    # `freya_tree_node_count` is the SHIM's own registry size, and it grew by
    # the three elements the accessor created plus the host.
    let nodesAfterMount = freya_tree_node_count()
    ...
    check freya_tree_node_count() > nodesAfterMount
  ...
  test "test_control_freya_imperative_mount_does_not_rebuild_on_signal_write":
  ...
  test "test_control_freya_static_native_root_keeps_build_once_semantics":
    ...
    let handle = renderFreya(r, host,
      staticNativeRoot(proc(): FreyaElement = buildLabelTree(r, label.val)))
    check handle.renders == 1
    check contains(textContent(host), "ALPHA")
  ...
  test "test_render_native_freya_mount_callback_publishes_the_current_root":
    # The overload the EX-M14 Freya launcher uses: the headless launcher holds
    # the root itself and feeds it to its frame source, so it publishes
    # through the `mount` callback. (`renderFreyaIntoShimRoot` is the windowed
    # variant that pushes into `freya_set_root_element` instead.) The callback
    # must run on every effect pass.
    freya_reset_tree()
    resetCallbacks()
    let r = FreyaRenderer()
    let label = createSignal("ALPHA")
    var published: FreyaElement = nil
    var publishes = 0

    let handle = renderFreya(
      NativeRootAccessor[FreyaElement](proc(): FreyaElement =
        buildLabelTree(r, label.val)),
  ...
  test "test_render_native_freya_dispose_stops_the_render_effect":
    freya_reset_tree()
    resetCallbacks()
    let r = FreyaRenderer()
    let host = r.createElement("div")
